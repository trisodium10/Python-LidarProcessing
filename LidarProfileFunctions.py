...
#        fnc = netcdf.netcdf_file(ncfilename, 'w')
        ...
        if not nc_error:
            ...
            if file_exists:
                fnc.history = fnc.history + "\nModified " + datetime.datetime.today().strftime("%d/%m/%y")     
            else:
...
def polyfit_with_fixed_points(n, x, y, xf, yf):
    """
    polyfit_with_fixed_points(n, x, y, xf, yf)
    returns a list of polynomial coefficients for a fit to data x,y
    with order n.  The fit is forced to pass through points xf and yf
